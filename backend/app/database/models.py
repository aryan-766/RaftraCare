...
class SubscriptionPlan(Base):
    ...
    tier: Mapped[SubscriptionTier] = mapped_column(sa.Enum(SubscriptionTier, native_enum=False), unique=True, nullable=False)
...
class HospitalSubscription(Base):
    ...
    status: Mapped[SubscriptionStatus] = mapped_column(sa.Enum(SubscriptionStatus, native_enum=False), default=SubscriptionStatus.TRIALING)
...
class HospitalStaff(Base):
    ...
    role: Mapped[UserRole] = mapped_column(sa.Enum(UserRole, native_enum=False), default=UserRole.RECEPTIONIST)
...
class Patient(Base):
    ...
    first_name: Mapped[str] = mapped_column(sa.String(100), nullable=False)
    last_name: Mapped[str] = mapped_column(sa.String(100), nullable=False)
    gender: Mapped[Gender] = mapped_column(sa.Enum(Gender, native_enum=False), nullable=False)
...
class Encounter(Base):
    ...
    type: Mapped[EncounterType] = mapped_column(sa.Enum(EncounterType, native_enum=False), default=EncounterType.OPD)
...
class Appointment(Base):
    ...
    status: Mapped[AppointmentStatus] = mapped_column(sa.Enum(AppointmentStatus, native_enum=False), default=AppointmentStatus.SCHEDULED)
    reason: Mapped[Optional[str]] = mapped_column(sa.Text)
    priority: Mapped[Priority] = mapped_column(sa.Enum(Priority, native_enum=False), default=Priority.NORMAL)
...
class QueueToken(Base):
    ...
    token_number: Mapped[int] = mapped_column(sa.Integer, nullable=False)
    status: Mapped[QueueStatus] = mapped_column(sa.Enum(QueueStatus, native_enum=False), default=QueueStatus.BOOKED)
    room_number: Mapped[Optional[str]] = mapped_column(sa.String(20))
...
class Ward(Base):
    ...
    hospital_id: Mapped[str] = mapped_column(sa.String(36), sa.ForeignKey("hospitals.id", ondelete="CASCADE"), nullable=False)
    name: Mapped[str] = mapped_column(sa.String(100), nullable=False)
    type: Mapped[WardType] = mapped_column(sa.Enum(WardType, native_enum=False), default=WardType.GENERAL)
...
class Bed(Base):
    ...
    status: Mapped[BedStatus] = mapped_column(sa.Enum(BedStatus, native_enum=False), default=BedStatus.AVAILABLE)
...
class Admission(Base):
    ...
    status: Mapped[AdmissionStatus] = mapped_column(sa.Enum(AdmissionStatus, native_enum=False), default=AdmissionStatus.ADMITTED)
...
class Prescription(Base):
    ...
    patient_id: Mapped[str] = mapped_column(sa.String(36), sa.ForeignKey("patients.id", ondelete="CASCADE"), nullable=False)
    doctor_id: Mapped[str] = mapped_column(sa.String(36), sa.ForeignKey("hospital_staff.id"), nullable=False)
    status: Mapped[PrescriptionStatus] = mapped_column(sa.Enum(PrescriptionStatus, native_enum=False), default=PrescriptionStatus.ACTIVE)
    notes: Mapped[Optional[str]] = mapped_column(sa.Text)
...
class LabOrder(Base):
    ...
    status: Mapped[LabOrderStatus] = mapped_column(sa.Enum(LabOrderStatus, native_enum=False), default=LabOrderStatus.ORDERED)
...
class Invoice(Base):
    ...
    status: Mapped[InvoiceStatus] = mapped_column(sa.Enum(InvoiceStatus, native_enum=False), default=InvoiceStatus.DRAFT)
...
class InvoiceItem(Base):
    ...
    item_type: Mapped[InvoiceItemType] = mapped_column(sa.Enum(InvoiceItemType, native_enum=False), default=InvoiceItemType.MISCELLANEOUS)
...
class Payment(Base):
    ...
    method: Mapped[PaymentMethod] = mapped_column(sa.Enum(PaymentMethod, native_enum=False), default=PaymentMethod.CASH)
    status: Mapped[PaymentStatus] = mapped_column(sa.Enum(PaymentStatus, native_enum=False), default=PaymentStatus.PENDING)
...
class AuditLog(Base):
    ...
    action: Mapped[AuditAction] = mapped_column(sa.Enum(AuditAction, native_enum=False), nullable=False)
