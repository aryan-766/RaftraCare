...
// ============================================================================
// PURE THREE.JS SHADER & MATERIAL UTILITIES (High Performance & Smooth Load)
// ============================================================================
...
export const Beams: FC<BeamsProps> = ({
  ...
  useEffect(() => {
    ...
    // 2. Renderer (Optimized pixel ratio for butter-smooth framerates)
    ...
    // 3. Lights & Group (Tuned for soft, non-glaring elegance)
    const group = new THREE.Group();
    group.rotation.z = THREE.MathUtils.degToRad(rotation);
    scene.add(group);

    // Subtle specular main light
    const dirLight1 = new THREE.DirectionalLight(lightColor, 1.35);
    dirLight1.position.set(0, 4, 12);
    group.add(dirLight1);

    // Soft cyan accent rim light
    const dirLight2 = new THREE.DirectionalLight("#38bdf8", 0.75);
    dirLight2.position.set(-6, 5, 8);
    group.add(dirLight2);

    // Ambient light tuned down to preserve deep contrast
  ...
  vec3 getNormal(vec3 pos) {
    ...
    gl_FragColor.rgb -= randomNoise / 14. * uNoiseIntensity;`,
      },
      material: { fog: false },
      uniforms: {
    ...
    // 5. Geometry (Optimized to 45 height segments for 60% less vertex overhead)
    ...
    const animate = (currentTime: number) => {
      ...
      if (beamMaterial.uniforms.time) {
        beamMaterial.uniforms.time.value = elapsedSeconds;
      }
  ...
  return (
    <div
      ref={containerRef}
      className={`w-full h-full relative overflow-hidden transition-opacity duration-700 ease-out ${
